...
void Map::destroyMap()
{
    ...
    for(unsigned int i = 0; i< Teleporte::TeleporteList.size();i++){
        Teleporte::TeleporteList[i]->destroy();
    }
...
{
    return loadMapWithStr(filename);
///DEPRECIADO, transmidido para loadMapWithStr();
/*
    if(tileset == NULL) {
        printf("tileset nao definido.\nDefina o tileset antes de carregar o mapa.\n");
        return false;
    }
    ...
    for (int y = 0; y < layer_height; y++)
    {
        ...
        {   // Le Tiles e joga-os no TileList
            Tile tempTile;
            fscanf(file, "%d:%d ", &tempTile.id, &tempTile.tipo);
    ...
    while(fscanf(file, "%s %d,%d,%d,%d,%d,%d\n",str,&tx,&ty,&tw,&th,&tdx,&tdy) == 7)
    {
        char* pString = (char*) malloc (sizeof(char)*80);
        strncpy(pString, str,sizeof(str) - 1);
        Teleporte* t = new Teleporte(pString,tx,ty,tw,th,tdx,tdy);
        t->addLists();
    }
    //toca musica
    if(fscanf(file,"%d", &tx) == 1) {
...
bool Map::loadMapWithStr(std::string filename)
{
    destroyMap();
...
void Map::render(int mapX, int mapY, bool isLayerImg)
{
    ...
    if (background != NULL)
        background->render(mapX-Camera::CameraControl.getX()*0.5f, mapY-Camera::CameraControl.getY()*0.7f, NULL);
